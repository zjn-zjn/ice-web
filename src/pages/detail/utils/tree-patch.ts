...
function generateLabelName(nodeId: number, nodeType: number, confName?: string, name?: string): string {
  let label: string
...
function findAllByNodeId(root: ChildrenItem, nodeId: number): ChildrenItem[] {
...
function reindexChildren(parent: ChildrenItem): void {
...
export function patchUpdateProps(
  ...
  nodeId: number,
  props: Partial<{
    inverse: boolean; timeType: number; start: number; end: number;
  ...
  for (const n of nodes) {
    ...
    if (props.confField !== undefined) {
      n.showConf.confField = props.confField
      // Sync parsed field values back into nodeInfo so re-opening the form shows updated data
      if (n.showConf.nodeInfo) {
        try {
          const parsed = JSON.parse(props.confField)
          const syncFields = (fields?: { field: string; value: string; valueNull: boolean }[]) => {
            if (!fields) return
            for (const f of fields) {
              if (f.field in parsed) {
                const v = parsed[f.field]
                f.valueNull = v === null
                f.value = v === null ? '' : (typeof v === 'object' ? JSON.stringify(v) : String(v))
              }
            }
          }
          syncFields(n.showConf.nodeInfo.iceFields)
          syncFields(n.showConf.nodeInfo.hideFields)
        } catch {}
      }
    }
